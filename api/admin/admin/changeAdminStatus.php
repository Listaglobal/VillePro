...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($data->user_id)) {
        $user_id = $utility_class_call::escape($data->user_id);
    }
    //pass token
    ...
    // Validate input
    if ($utility_class_call::validate_input($user_id)) {
    ...
    if (!$utility_class_call::checkIfExist("admin", "user_id", $user_id)) {
    ...
    $updateStatus = $utility_class_call::changeStatus("admin", $status, "user_id", $user_id);
    ...
    $maindata = [];
    $text = $api_response_class_call::$statusChanged;
