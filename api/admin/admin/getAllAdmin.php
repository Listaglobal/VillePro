...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (isset($_GET['user_id'])) {
        $single_post = true;
        $status = $utility_class_call::escape($_GET['user_id']);
        $sortQuery .= ' AND ' . $api_admin_table_class_call::tableName . '.user_id = ?';
        $paramString .= "s";
        $params[] = $status;
    } else {
        $single_post = false;
    ...
    if (isset($_GET['search'])) {
        $tableName = $api_admin_table_class_call::tableName;
    ...
    $AllReview = $api_admin_table_class_call::getAllAdmin($page_no, $offset, $noPerPage, $searchQuery, $sortQuery, $paramString, $params);
