 ...
 * PHP version 5.4
 */
class Available_Table extends Config\DB_Connect
{
    /**
     ...
     */
    // APi functions
    public const  tableName = "available";
    ...
    public static function getAllAvailable($page, $offset, $noPerPage, $searchQuery, $sortQuery, $paramString, $params)
    {
        ...
        $query = "SELECT $tableName.*, staff.fullname as staff_fullname, staff.profile_pic as profile_pic, staff.skills as staff_skills FROM $tableName LEFT JOIN staff ON $tableName.user_id = staff.staff_id WHERE $tableName.id > ? $sortQuery $searchQuery";
    ...
    {
        $connect = static::getDB();
        $trackid =  Utility_Functions::generateUniqueShortKey("available", "trackid");
        $status = 1;
        ...
        // `trackid`, `user_id`, `daysfrom`, `daysto`, `work_hour`, `status`,

        $query = "INSERT INTO `available`(`trackid`, `status`, `user_id`, `daysfrom` , `daysto`, `work_hour` ) VALUES (?, ?, ?, ?, ?, ?)";
