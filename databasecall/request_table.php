...
class Request_Table extends Config\DB_Connect
{
    ...
    public static function getAllRequest($page, $offset, $noPerPage, $searchQuery, $sortQuery, $paramString, $params)
    {
        ...
        $query = "SELECT $tableName.*, staff.fullname as staff_fullname, staff.profile_pic as profile_pic, staff.skills as staff_skills FROM $tableName LEFT JOIN staff ON $tableName.staff_id = staff.staff_id WHERE $tableName.id > ? $sortQuery $searchQuery";
    ...
    public static function addRequest($data)
    {
        ...
        $query = "INSERT INTO `request`(`trackid`, `status`, `staff_id`, `reason`, `days` , `daysto` ) VALUES (?, ?, ?, ?, ?, ?)";
